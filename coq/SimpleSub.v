...
(* (5) If split(V) => V1 | V2 then V1/V => ok and V2/V => ok CANNOT PROVE DIRECTLY *)
...
Lemma psub_neg : forall V1 V2 A,
    V1 <p A -> isNegTyp V1 -> lc_typ V2 -> V2 <p A.
Proof.
  introv Sub Neg Lc. gen V2.
...
Proof.
  ...
  induction HS; intros.
  - applys PSub_UnionL; try applys* psub_neg; eauto.
  - applys PSub_UnionR; try applys* psub_neg; eauto.
  - inverts Spl; solve_false.
    eapply PSub_UnionL in HP1. eapply PSub_UnionR in HP2.
    applys* psub_splu_l_trans. all: auto_lc.
Qed.

Lemma sim_psub_inv_1 : forall V A B C,
   sim A B -> splu V A B -> isValTyp V -> isValTyp A -> A <p C -> B <p C \/ isValTyp B.
Proof with solve_false.
  introv HS Spl Val1 Val2 HP.
  indTypSize (size_typ A + size_typ B + size_typ C).
  inverts HS; intros.
  - left. try applys* psub_neg; eauto.
  - right*.
  - inverts Spl... inverts Val1... inverts Val2...
    inverts HP...
    (* + forwards [?|?]: IH; [ | | | eassumption | | eassumption | ..]; try eassumption; elia. *)
    (*   all: eauto. *)
    all: try (forwards [?|?]: IH; [ | | eassumption | eassumption | eassumption | ..]; try eassumption; elia)...
    all: try solve [ left* ].
    all: try solve [ right* ].
    all: eapply Sim_In in H; eapply SpU_in in H2.
    1-2: try (forwards [?|?]: IH; [ | | eassumption | eassumption | ..]; try eassumption; elia; eauto)...
    + eapply VTypIn in H1. eapply VTypIn in H3.
      forwards [?|?]: IH H H2 H0; try eassumption; elia.
      all: forwards [?|?]: IH H H2 H4; try eassumption; elia.
      all: eauto.
Qed.

Lemma sim_psub_inv_2 : forall V A B C,
   sim A B -> splu V A B -> isValTyp V -> isValTyp B -> B <p C -> A <p C \/ isValTyp A.
Admitted.

Lemma psub_splu_inv_1 : forall V A B C,
    splu V A B -> isValTyp V -> isValTyp A -> A <p C -> B <p C \/ isValTyp B.
Proof.
  intros.
  assert (HS: sim A B). { applys sim2similar. unfold similar. eauto. }
  applys sim_psub_inv_1 HS; eassumption.
Qed.

Lemma psub_splu_inv_2 : forall V A B C,
    splu V A B -> isValTyp V -> isValTyp B -> B <p C -> A <p C \/ isValTyp A.
Proof.
  intros.
  assert (HS: sim A B). { applys sim2similar. unfold similar. eauto. }
  applys sim_psub_inv_2 HS; eassumption.
Qed.
(******************************************************************************)
(* CANNOT PROVE DIRECTLY; COUNTER EXAMPLE EXISTS *)
Lemma psub_splu_l_1 : forall A A1 A2 B,
    splu A A1 A2 -> A1 <p B -> isValTyp A -> isValTyp A1 -> A <p B.
Proof with solve_false.
  introv Spl Sub Val1 Val2.
  indTypFtySize (size_typ A + size_typ B).
  inverts Spl; inverts_typ...
  all: try solve [ applys psub_neg; [ | eassumption | ..]; [eassumption | ..]; eauto ].
  (* the c A | Bneg case come again *)
Abort.
...
Proof with try eassumption; elia.
  ...
  inverts Sub; try solve [inverts Val]; inverts_typ; intros; solve_false.
  all: eauto.
  all: try solve [forwards: IH; [ | eassumption | .. ]; try eassumption; elia; eauto].
  - (* intersect on the right *) forwards: IH H0... forwards: IH H1... applys~ psub_spli_r H.
  - applys~ psub_neg A1. applys* IH...
  - applys~ psub_neg A2. applys* IH...
  - (* two rcd *) destruct H6 as [HV | HV]; forwards~ : IH HV A...
    + forwards [?|HV']: psub_splu_inv_1 H...
      2: forwards: IH HV' A...
      all: applys psub_splu_l_trans...
    + forwards [?|HV']: psub_splu_inv_2 H...
      2: forwards: IH HV' A...
      all: applys psub_splu_l_trans...
Qed.

(* (5) If split(V) => V1 | V2 then V1/V => ok and V2/V => ok *)
Lemma psub_val_splu_rev_both : forall A A1 A2,
    splu A A1 A2 -> isValTyp A -> (isValTyp A1 -> A1 <p A) /\ (isValTyp A2 -> A2 <p A).
Proof.
  introv Spl Val.
  split; intro Val2; applys~ sub2psub; convert2asub; eauto.
Qed.

Lemma psub_val_splu_l_1 : forall A A1 A2 B,
    splu A A1 A2 -> isValTyp A -> A1 <p B -> isValTyp A1 -> A <p B.
(* Lemma psub_val_splu_l_1 : forall A A1 A2, *)
(*     splu A A1 A2 -> isValTyp A -> isValTyp A1 -> A1 <p A. *)
Proof.
  introv Spl Val1 HP Val2.
  applys psub_trans. forwards (?&_): psub_val_splu_rev_both Spl Val1.
  (* CANNOT BE PROVED BY THIS WAY *)
Abort.

(* Lemma psub_val_splu_l_2 : forall A A1 A2, *)
(*     splu A A1 A2 -> isValTyp A -> isValTyp A2 -> A2 <p A. *)
(* Proof. *)
(*   introv Spl Val. applys psub_val_splu_rev_both Spl Val. *)
(* Qed. *)
...
Proof.
  ...
  inductions Sub; try solve [inverts Neg1]; intros; eauto using psub_neg.
Qed.
...
Proof.
  ...
  induction Neg.
  - (* arrow *) inverts Sub. applys~ NSub_Fun.
    (* BROKEN BECAUSE psub_splu_l_1 is not true *)
Abort.
(*     forwards~ : psub_val_splu_l_1 Spl. H6. *)
(*   - (* forall *) inverts* Sub. *)
(*   - (* and *) *)
...
Lemma sim_apply_inv_1 : forall V A B C A',
   sim A B -> splu V A B -> isValTyp V -> isValTyp A -> ApplyTy C A A'
   -> (exists B', ApplyTy C B B') \/ isValTyp B.
Proof with try eassumption; elia; solve_false.
  introv HS Spl Val1 Val2 HP.
  indTypFtySize (size_typ A + size_typ B + size_typ C).
  inverts HS; intros.
  - left. exists. admit. (* ??? *)
    (* try applys* psub_neg; eauto. *)
  - right*.
  (* - inverts HP... eapply Sim_In in H. *)
  (*   forwards [(?&?)|?]: IH H Spl H1... *)
  (*   all: forwards [(?&?)|?]: IH H Spl H2... *)
  (*   + left. exists*. *)
  (*     all: right*. *)
Admitted.

(* wrongly stated ? *)
Lemma sim_apply_inv_1_old : forall V A B C A',
   sim A B -> splu V A B -> isValTyp V -> isValTyp A -> ApplyTy A C A'
   -> (exists B', ApplyTy B C B') \/ isValTyp B.
Proof with try eassumption; elia; solve_false.
  introv HS Spl Val1 Val2 HP.
  indTypFtySize (size_typ A + size_typ B + size_Fty C).
  inverts HS; intros.
  - left. exists. admit. (* ??? *)
    (* try applys* psub_neg; eauto. *)
  - right*.
...
Lemma apply2nsub : forall A F C,
    (* isNegTyp A -> *)
    isValFty F -> ApplyTy A F C -> A <n F.
(* Admitted. *)
(* lack inversion of negtyp *)
Proof.
  introv Val App.
  indTypFtySize (size_Fty F + size_typ A).
  inverts* App.
  all: try (forwards: IH H0 ; try eassumption; try eauto; now elia).
  all: try solve [forwards: IH H0 ; try eassumption; try eauto; elia;
            forwards: IH H1 ; try eassumption; try eauto; elia].
  (* - (* forall *) eauto. *)
  - (* arrow *)
    inverts Val. inverts* H0. constructor~.
    applys~ sub2psub.
  - (* union *) (* maybe use similar trick? *)
    inverts Val.
    assert (Sim: sim B1 B2). {
      applys sim2similar. unfold similar. eauto.
    }
    forwards~ [?|?]: valtyp_splu_inv_aux H.
    + forwards~ [(?&?)|?]: sim_apply_inv_1 Sim H H0.
      (* applys ApplyTyUnionArg. *)
Admitted.
    (* inverts* Neg. forwards~ : IHApp1. *)
(*   - (* union *) inverts* Neg. *)
